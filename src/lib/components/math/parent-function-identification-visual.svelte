...
<script lang="ts">
	...
	const isGuessLocked = $derived(selectedParent !== null);
	...
	const transformParameterTex = $derived(`a=${card.a},\\;h=${card.h},\\;k=${card.k}`);
	...
	function getChoiceButtonClass(parentId: ParentId) {
		...
		if (parentId === selectedParent) {
			return `${baseClass} border-rose-500/60 bg-rose-100/70 text-rose-900`;
		}
...
<div class="space-y-4">
	...
	<div class="space-y-3 rounded-xl border border-border/70 bg-background/75 p-3">
		<p class="text-sm text-muted-foreground">
			Match the graph to its parent function. This card uses:
			<MathExpression
				math="y=a\\,f(x-h)+k"
		...
		<p class="text-sm text-muted-foreground">
			<MathExpression math={transformParameterTex} class="font-semibold text-foreground" />
		</p>
		<div class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-3">
			<p class="rounded-lg border border-border/70 bg-background/80 px-3 py-2">
				Vertical effect: <span class="font-semibold text-foreground">{verticalScaleText}</span>
			</p>
			<p class="rounded-lg border border-border/70 bg-background/80 px-3 py-2">
				Horizontal shift: <span class="font-semibold text-foreground">{horizontalShiftText}</span>
			</p>
			<p class="rounded-lg border border-border/70 bg-background/80 px-3 py-2">
				Vertical shift: <span class="font-semibold text-foreground">{verticalShiftText}</span>
			</p>
		</div>
	</div>
		...
		>
			<p class="font-semibold">
				...
				{:else}
					Not quite.
				{/if}
				This card comes from <span class="font-bold">{activeParent.label}</span>.
			</p>
			<p class="mt-1">
