<script module lang="ts">
	...
	export const toolMeta: MathToolMeta = requireInteractiveToolMetaById('simple-interest-growth');
</script>
...
<script lang="ts">
	...
	let annualRatePercent = $state(5);
	let horizonYears = $state(20);
	...
	function valueAt(timeYears: number) {
		return safePrincipal * (1 + annualRateDecimal * timeYears);
	}
	...
	const curvePoints = $derived.by(() => {
		const samples = Math.max(24, Math.min(360, safeHorizonYears * 16));
		const points: string[] = [];
...
<div class="space-y-4">
	...
	<svg
		...
		role="img"
		aria-label="Simple interest growth graph"
	>
